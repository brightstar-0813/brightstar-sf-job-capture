...
function buildSearchUrl(q) {
  ...
  return `https://www.ziprecruiter.com/Jobs/Remote-${encodeURIComponent(
    String(q || "Salesforce").replace(/\s+/g, "-")
  )}`;
}
...
export async function searchZiprecruiterJobs(browser) {
  ...
  try {
    for (const q of queries) {
      ...
      console.log(
        `[ziprecruiter] "${q}" listing links: ${stubs.length}` +
      ...
      if (!relevant.length) {
        console.warn(
          `[ziprecruiter] "${q}" no job links found (page may be JS-gated)`
        );
        continue;
      }
      ...
      for (let i = 0; i < relevant.length; i += 1) {
        ...
        seen.add(stub.url);
        console.log(
          `[ziprecruiter] detail ${i + 1}/${relevant.length} (${q})`
        );
        try {
          ...
          const id = `ziprecruiter_${idMatch ? idMatch[1] : kept.length}`;
