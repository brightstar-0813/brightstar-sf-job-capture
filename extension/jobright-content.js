...
/** Keep Indeed jk= / Greenhouse token; Lever /apply → posting; reject shells. */
function normalizeExternalJobUrl(raw) {
...
function pickJobrightJobUrl(jr) {
...
function buildDescription(jr) {
  ...
  const must = bulletBlock(
    "Requirements",
    Array.isArray(quals?.mustHave)
      ? quals.mustHave
